...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (body.startDateTime !== undefined) {
      ...
      if (!d) return NextResponse.json({ success: false, error: "Invalid startDateTime" }, { status: 400 });
      update.startDateTime = d;
    }
    if (body.endDateTime !== undefined) {
      const d = parseDateMaybe(body.endDateTime);
      if (!d) return NextResponse.json({ success: false, error: "Invalid endDateTime" }, { status: 400 });
      update.endDateTime = d;
    }
    if (update.startDateTime && update.endDateTime && +update.endDateTime <= +update.startDateTime) {
      return NextResponse.json({ success: false, error: "endDateTime must be after startDateTime" }, { status: 400 });
    }

    // Enumerations (optional)
    if (body.occasion !== undefined) {
      if (!isOneOf(body.occasion, Occasion)) {
        return NextResponse.json({ success: false, error: "Invalid occasion" }, { status: 400 });
      }
    ...
    if (body.dining !== undefined) {
      if (!isOneOf(body.dining, Dining)) {
        return NextResponse.json({ success: false, error: "Invalid dining" }, { status: 400 });
      }
      update.dining = body.dining;
    }
    if (body.budget !== undefined) {
      if (!isOneOf(body.budget, Budget)) {
        return NextResponse.json({ success: false, error: "Invalid budget" }, { status: 400 });
      }
    ...
    if (body.dietAllergies !== undefined) update.dietAllergies = String(body.dietAllergies || "").trim();

    if (body.flowers !== undefined) update.flowers = Boolean(body.flowers);
    if (body.cake !== undefined) update.cake = Boolean(body.cake);

    if (body.personalNote !== undefined) update.personalNote = String(body.personalNote || "").trim();

    if (body.name !== undefined) {
      const name = String(body.name || "").trim();
      if (name && name.length < 2) {
        return NextResponse.json({ success: false, error: "Name must be at least 2 chars" }, { status: 400 });
      }
      update.name = name;
    }
